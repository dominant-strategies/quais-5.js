...
function generate() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const versions = Object.keys(lines.reduce((accum, line, index) => {
            const match = line.match(/^quais\/v([^ ]*)/);
            if (match) {
                if (firstLine == null) {
                    firstLine = index;
                }
        ...
        const version = local.getPackage("quais").version;
        ;
        const published = yield npm.getPackage("quais");
        ...
        const newTitle = `quais/v${version} (${(0, utils_1.getDateTime)(new Date())})`;
        ...
        changes.forEach((change) => {
            ...
            let commit = `[${change.commit.substring(0, 7)}](https://github.com/quais-io/quais.js/commit/${change.commit})`;
            let link = commit;
            if (linkMatch) {
                body = body.replace(/ *(\(.*#.*)\) */, "");
                link = linkMatch[2].replace(/#([0-9]+)/g, (all, issue) => {
                    return `[#${issue}](https://github.com/quais-io/quais.js/issues/${issue})`;
                }) + "; " + commit;
            }
