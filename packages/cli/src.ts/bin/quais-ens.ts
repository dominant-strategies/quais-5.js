...
abstract class EnsPlugin extends Plugin {
    ...
    constructor() {
        super();
        quais.utils.defineReadOnly(this, "_ethAddressCache", { });
    }

    getEns(): quais.Contract {
        return new quais.Contract(this.network.ensAddress, ensAbi, this.accounts[0] || this.provider);
    }

    async getResolver(nodehash: string): Promise<quais.Contract> {
        if (!this._ethAddressCache[nodehash]) {
            this._ethAddressCache[nodehash] = await this.getEns().resolver(nodehash);
        }
        return new quais.Contract(this._ethAddressCache[nodehash], resolverAbi, this.accounts[0] || this.provider);
    }

    async getEthInterfaceAddress(interfaceId: string): Promise<string> {
        let ethNodehash = quais.utils.namehash("eth");
    ...
    async getEthController(): Promise<quais.Contract> {
        let address = await this.getEthInterfaceAddress(InterfaceID_Controller);
        return new quais.Contract(address, ethControllerAbi, this.accounts[0] || this.provider);
    }

    async getEthLegacyRegistrar(): Promise<quais.Contract> {
        let address = await this.getEthInterfaceAddress(InterfaceID_Legacy);
        return new quais.Contract(address, ethLegacyRegistrarAbi, this.accounts[0] || this.provider);
    }

    async getEthRegistrar(): Promise<quais.Contract> {
        //let address = await this.getEthInterfaceAddress(InterfaceID_ERC721);
        let address = await this.getEns().owner(quais.utils.namehash("eth"));
        return new quais.Contract(address, ethRegistrarAbi, this.accounts[0] || this.provider);
    }
}
...
class LookupPlugin extends EnsPlugin {
    ...
    async run(): Promise<void> {
        ...
        for (let i = 0; i < this.names.length; i++) {
            ...
            let nodehash = quais.utils.namehash(name);
            ...
            if (comps.length === 2 && comps[1] === "eth") {
                details.Labelhash = quais.utils.id(comps[0].toLowerCase()); // @TODO: nameprep
                ...
                if (!details.Available) {
                    try {
                        ...
                        if (ownerOf !== quais.constants.AddressZero) {
                            details.Registrant = ownerOf;
                            details.Registrar = "Permanent";
                        }
                    } catch (error) {
                        let entry = await legacyRegistrar.entries(details.Labelhash);
                        let deed = new quais.Contract(entry.owner, deedAbi, this.provider);

                        details.Registrant = await deed.owner();
                        details.Registrar = "Legacy";
                        details["Deed Value"] = (quais.utils.formatEther(entry.value) + " ether");
                        details["Highest Bid"] = (quais.utils.formatEther(entry.highestBid) + " ether");
                    }
                }
            }

            if (resolverAddress) {
                let resolver = new quais.Contract(resolverAddress, resolverAbi, this.provider);
                ...
                let content = await resolver.contenthash(nodehash).then((hash: string) => {
                    if (hash === "0x") { return ""; }
                    if (hash.substring(0, 10) === "0xe3010170" && quais.utils.isHexString(hash, 38)) {
                        return Base58.encode(quais.utils.hexDataSlice(hash, 4)) + " (IPFS)";
                    }
...
abstract class AccountPlugin extends EnsPlugin {
    ...
    static getHelp(): Help {
        return logger.throwError("subclasses must implement this", quais.errors.UNSUPPORTED_OPERATION, {
            operation: "getHelp"
        });
    }

    async _setValue(key: string, value: string): Promise<void> {
        quais.utils.defineReadOnly<any, any>(this, key, value);
        if (key === "name") {
            await this._setValue("nodehash", quais.utils.namehash(value));
        }
    }
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        let helpLine = quais.utils.getStatic<() => Help>(this.constructor, "getHelp")().name;
...
abstract class ControllerPlugin extends AccountPlugin {
    ...
    async prepareOptions(argParser: ArgParser): Promise<void> {
        ...
        if (secret) {
            ...
            this.salt = quais.utils.id(secret);
        }
        ...
        quais.utils.defineReadOnly(this, "duration", duration * (60 * 60 * 24));
    }
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        if (!this.salt) {
            ...
            this.salt = quais.utils.keccak256(signature);
        }
    }
}
...
class CommitPlugin extends ControllerPlugin {
    ...
    async run(): Promise<void> {
        ...
        this.dump("Commit: " + this.name, {
            ...
            Fee: quais.utils.formatEther(fee) + " (informational)",
...
class RevealPlugin extends ControllerPlugin {
    ...
    async run(): Promise<void> {
        ...
        this.dump("Reveal: " + this.name, {
            ...
            Fee: quais.utils.formatEther(fee),
        });
...
class SetSubnodePlugin extends AddressAccountPlugin {
    ...
    async run(): Promise<void> {
        ...
        await this.getEns().setSubnodeOwner(quais.utils.namehash(this.node), quais.utils.id(this.label), this.address);
    }
}
...
class SetNamePlugin extends AddressAccountPlugin {
    ...
    async run(): Promise<void> {
        ...
        const nodehash = quais.utils.namehash(this.address.substring(2) + ".addr.reverse");
...
class SetContentPlugin extends AccountPlugin {
    ...
    async _setValue(key: string, value: string): Promise<void> {
        if (key === "hash") {
            ...
            let multihash = quais.utils.concat([ "0xe3010170", bytes ]);
            await super._setValue("multihash", quais.utils.hexlify(multihash));
        }
        await super._setValue(key, value);
    }
...
class MigrateRegistrarPlugin extends AccountPlugin {
    ...
    readonly deedValue: quais.BigNumber;
    readonly highestBid: quais.BigNumber;
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        let entry: any = await ethLegacyRegistrar.entries(quais.utils.id(comps[0]));
    ...
    async run(): Promise<void> {
        ...
        this.dump("Migrate Registrar: " + this.name, {
            "Nodehash": this.nodehash,
            "Highest Bid": (quais.utils.formatEther(this.highestBid) + " ether"),
            "Deed Value": (quais.utils.formatEther(this.deedValue) + " ether"),
        });

        let legacyRegistrar = await this.getEthLegacyRegistrar();
        await legacyRegistrar.transferRegistrars(quais.utils.id(this.label));
    }
}
...
class TransferPlugin extends AccountPlugin {
    ...
    async run(): Promise<void> {
        ...
        let registrar = await this.getEthRegistrar();
        await registrar.safeTransferFrom(this.accounts[0].getAddress(), this.new_owner, quais.utils.id(this.label));
    }
}
...
class ReclaimPlugin extends AddressAccountPlugin {
    ...
    async _setValue(key: string, value: string): Promise<void> {
        if (key === "name") {
            ...
            try {
                ownerOf = await registrar.ownerOf(quais.utils.id(comps[0]));
            } catch (error) {
    ...
    async run(): Promise<void> {
        ...
        let registrar = await this.getEthRegistrar();
        await registrar.reclaim(quais.utils.id(this.label), this.address);
    }
}
...
class RenewPlugin extends EnsPlugin {
    ...
    async run(): Promise<void> {
        ...
        for (let i = 0; i < this.labels.length; i++) {
            ...
            const expiration = (await ethRegistrar.nameExpires(quais.utils.id(label))).toNumber();
            if (expiration === 0) {
            ...
            this.dump(`Renew: ${ label }.eth`, {
                ...
                "Fee":            `${ quais.utils.formatEther(fee) } (+10% buffer)`,
            });
