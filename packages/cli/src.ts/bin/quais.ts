...
function setupContext(path: string, context: any, plugin: Plugin) {
    ...
    context.quais = quais;
    context.version = quais.version;

    context.Contract = quais.Contract;
    context.ContractFactory = quais.ContractFactory;
    context.Wallet = quais.Wallet;

    context.providers = quais.providers;
    context.utils = quais.utils;

    context.abiCoder = quais.utils.defaultAbiCoder;

    context.BN = quais.BigNumber;
    context.BigNumber = quais.BigNumber;
    context.FixedNumber = quais.FixedNumber;

    context.getAddress = quais.utils.getAddress;
    context.getContractAddress = quais.utils.getContractAddress;
    context.getIcapAddress = quais.utils.getIcapAddress;

    context.arrayify = quais.utils.arrayify;
    context.concat = quais.utils.concat;
    context.hexlify = quais.utils.hexlify;
    context.zeroPad = quais.utils.zeroPad;

    context.joinSignature = quais.utils.joinSignature;
    context.splitSignature = quais.utils.splitSignature;

    context.id = quais.utils.id;
    context.keccak256 = quais.utils.keccak256;
    context.namehash = quais.utils.namehash;
    context.sha256 = quais.utils.sha256;

    context.parseEther = quais.utils.parseEther;
    context.parseUnits = quais.utils.parseUnits;
    context.formatEther = quais.utils.formatEther;
    context.formatUnits = quais.utils.formatUnits;

    context.randomBytes = quais.utils.randomBytes;
    context.constants = quais.constants;

    context.parseTransaction = quais.utils.parseTransaction;
    context.serializeTransaction = quais.utils.serializeTransaction;

    context.toUtf8Bytes = quais.utils.toUtf8Bytes;
    context.toUtf8String = quais.utils.toUtf8String;
}
...
class InitPlugin extends Plugin {
    ...
    async run(): Promise<void> {
        ...
        let wallet = quais.Wallet.createRandom();
...
class FundPlugin extends Plugin {
    ...
    async run(): Promise<void> {
        let url = "https:/" + "/api.quais.io/api/v1/?action=fundAccount&address=" + this.toAddress.toLowerCase();
        return quais.utils.fetchJson(url).then((data) => {
...
class InfoPlugin extends Plugin {
    ...
    async run(): Promise<void> {
        for (let i = 0; i < this.addresses.length; i++) {
            ...
            let { balance, nonce, code, reverse } = await quais.utils.resolveProperties({
            ...
            let info: any = {
                "Address": address,
                "Balance": (quais.utils.formatEther(balance) + " ether"),
...
class SendPlugin extends Plugin {
    ...
    async prepareOptions(argParser: ArgParser): Promise<void> {
        ...
        this.data = quais.utils.hexlify(argParser.consumeOption("data") || "0x");
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        this.value = quais.utils.parseEther(args[1]);
    }
...
class SweepPlugin extends Plugin {
    ...
    async run(): Promise<void> {
        ...
        let { balance, gasPrice, code } = await quais.utils.resolveProperties({
...
class WrapEtherPlugin extends Plugin {
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        this.value = quais.utils.parseEther(args[0]);
    ...
    async run(): Promise<void> {
        ...
        this.dump("Wrapping ether", {
            "From": address,
            "Value": quais.utils.formatEther(this.value)
        });

        let contract = new quais.Contract(WethAddress, WethAbi, this.accounts[0]);
...
class SendTokenPlugin extends Plugin {
    ...
    async prepareArgs(args: Array<string>): Promise<void> {
        ...
        this.contract = new quais.Contract(tokenAddress, Erc20Abi, this.accounts[0]);

        this.decimals = await this.contract.decimals();

        this.toAddress = await this.getAddress(args[1]);
        this.value = quais.utils.parseUnits(args[2], this.decimals);
    }
    ...
    async run(): Promise<void> {
        const info: { [ name: string ]: any } = {
            ...
            "Value": quais.utils.formatUnits(this.value, this.decimals)
        };
        ...
        let namePromise = this.contract.name().then((name: string) => {
            ...
            let contract = new quais.Contract(this.contract.address, Erc20AltAbi, this.contract.signer);
            contract.name().then((name: string) => {
                info["Token Name"] = quais.utils.parseBytes32String(name);
            }, (error: Error) => {
                throw error;
            });
        });
        ...
        let symbolPromise = this.contract.symbol().then((symbol: string) => {
            ...
            contract.symbol().then((symbol: string) => {
                info["Token Symbol"] = quais.utils.parseBytes32String(symbol);
            }, (error: Error) => {
                throw error;
            });
        });
    ...
    async run(): Promise<void> {
        ...
        result.forEach((contract, index) => {
            output[contract.name] = {
                ...
                interface: contract.interface.fragments.map((f) => f.format(quais.utils.FormatTypes.full)),
...
class DeployPlugin extends Plugin {
    ...
    async run(): Promise<void> {
        ...
        const factory = new quais.ContractFactory(codes[0].interface, codes[0].bytecode, this.accounts[0]);

        dump("Deploying:", {
            Contract: codes[0].name,
            Bytecode: codes[0].bytecode,
            Interface: codes[0].interface.fragments.map((f) => f.format(quais.utils.FormatTypes.full)),
