...
import { loadTests, randomNumber } from "@quais/testcases";
...
describe('Test HD Node Derivation is Case Agnostic', function () {
    ...
    tests.forEach((test) => {
        ...
        it("Normalizes case - " + test.name, function () {
            this.timeout(10000);
            let wordlist = (quais.wordlists)[test.locale];
...
describe('Test HD Mnemonic Phrases', function testMnemonic() {
    ...
    tests.forEach(function (test) {
        ...
        it(('converts mnemonic phrases - ' + test.name), function () {
            ...
            assert.equal(quais.utils.mnemonicToSeed(test.mnemonic, test.password), test.seed, 'Converts mnemonic to seed - ' + test.mnemonic + ':' + test.password);
            // Test default english
            if (test.locale === "en") {
                assert.equal(quais.utils.entropyToMnemonic(test.entropy), test.mnemonic, "Converts entropy to mnemonic " + test.name + " (default en)");
                assert.equal(quais.utils.mnemonicToEntropy(test.mnemonic), test.entropy, "Converts mnemonic to entropy - " + test.mnemonic + " (default en)");
            }
            let wordlist = (quais.wordlists)[test.locale];
            ...
            assert.equal(quais.utils.mnemonicToEntropy(test.mnemonic, wordlist), test.entropy, 'Converts mnemonic to entropy - ' + test.mnemonic);
        });
    });
});
