...
function equals(actual, expected) {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!quais_1.quais.utils.isHexString(actual)) {
            return false;
        }
        actual = quais_1.quais.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        var actualAddress = quais_1.quais.utils.getAddress(actual);
        var expectedAddress = quais_1.quais.utils.getAddress(expected);
...
function getValues(object, named) {
    ...
    switch (object.type) {
        ...
        case 'string':
            return object.value;
        case 'buffer':
            return quais_1.quais.utils.arrayify(object.value);
        case 'tuple':
...
describe('ABI Coder Encoding', function () {
    var coder = quais_1.quais.utils.defaultAbiCoder;
...
describe('Test Contract Events', function () {
    ...
    tests.forEach(function (test, index) {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function () {
            this.timeout(120000);
            var iface = new quais_1.quais.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function () {
            ...
            test.normalizedValues.forEach(function (expected, index) {
                if (test.indexed[index]) {
                    assert_1.default.ok((quais_1.quais.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                }
                else {
...
describe('Test Interface Signatures', function () {
    ...
    tests.forEach(function (test) {
        it('derives the correct signature - ' + test.name, function () {
            var iface = new quais_1.quais.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function () {
        var iface = new quais_1.quais.utils.Interface(["function transfer(address from, uint amount)"]);
        [
            "transfer",
    ...
    it('parses transaction function', function () {
        var iface = new quais_1.quais.utils.Interface(["function transfer(address from, uint amount)"]);
        // Transaction to transfer QRC20 token
        var rawTx = "";
        var tx = quais_1.quais.utils.parseTransaction(rawTx);
        var descr = iface.parseTransaction(tx);
        assert_1.default.equal(descr.args[0], '0x193399fA97aE9762A186E921582cedb0987D9470', 'parsed tx - args[0]');
        assert_1.default.equal(descr.args[1].toString(), '100000000000000000', 'parsed tx - args[1]');
...
describe('Test Number Coder', function () {
    ...
    it('encodes zero', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: quais_1.quais.constants.Zero }
        ];
    ...
    it('encodes one', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: quais_1.quais.constants.One }
        ];
        var expected = oneHex;
        ['uint8', 'uint256', 'int8', 'int256'].forEach(function (type) {
            ZeroValues.forEach(function (value) {
    ...
    it('encodes negative one', function () {
        var Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: quais_1.quais.constants.NegativeOne }
        ];
        var expected = maxHex;
    ...
    it('encodes full uint8 range', function () {
        for (var i = 0; i < 256; i++) {
            ...
            expected += quais_1.quais.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function () {
        for (var i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += quais_1.quais.utils.hexlify(256 + i).substring(2);
            }
            else {
...
describe('Test Filters', function () {
    ...
    function doTest(test) {
        it(test.name, function () {
            var iface = new quais_1.quais.utils.Interface([test.signature]);
...
describe('Test EIP-838 Error Codes', function () {
    ...
    var testaddr = "0x044f7c17c62f566302de11b3ac0875b0292031e1";
    it("testError1", function () {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        provider = new quais_1.quais.providers.JsonRpcProvider("https://rpc.cyprus1.colosseum.quaiscan.io");
                    ...
                    case 4:
                        ...
                        assert_1.default.equal(error_1.errorArgs[0].toLowerCase(), testaddr, "error.errorArgs[0]");
                        assert_1.default.equal(error_1.errorArgs.addr.toLowerCase(), testaddr, "error.errorArgs.addr");
...
describe("Additional test cases", function () {
    ...
    it("allows addresses without the 0x", function () {
        var iface = new quais_1.quais.utils.Interface([
