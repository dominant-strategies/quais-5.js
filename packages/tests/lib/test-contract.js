...
function equals(name, actual, expected) {
    ...
    if (typeof (actual) === 'object') {
        if (expected.indexed) {
            assert_1.default.ok(quais_1.quais.Contract.isIndexed(actual), 'index property has index - ' + name);
            if (expected.hash) {
...
describe("Test Contract Transaction Population", function () {
    ...
    var contract = new quais_1.quais.Contract(testAddress, abi);
    var contractConnected = contract.connect(quais_1.quais.getDefaultProvider());
