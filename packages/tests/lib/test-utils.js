...
describe('Test Contract Address Generation', function () {
    ...
    var getContractAddress = quais_1.quais.utils.getContractAddress;
    var Tests = [
    ...
    Tests.forEach(function (test) {
        it(('Computes the transaction address - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.equal(getContractAddress(test.tx.from, test.tx.nonce, test.tx.data), test.address, 'computes the transaction address');
        });
    });
});
describe('Test RLP Coder', function () {
    ...
    tests.forEach(function (test) {
        it(('RLP coder encoded - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.equal(quais_1.quais.utils.RLP.encode(test.decoded), test.encoded, 'RLP encoded - ' + test.name);
        });
    });
    tests.forEach(function (test) {
        it(('RLP coder decoded - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.ok(equals(quais_1.quais.utils.RLP.decode(test.encoded), test.decoded), 'RLP decoded - ' + test.name);
        });
    });
});
describe('Test Unit Conversion', function () {
    ...
    tests.forEach(function (test) {
        var wei = quais_1.quais.BigNumber.from(test.wei);
        it(('parses ' + test.ether + ' ether'), function () {
            assert_1.default.ok(quais_1.quais.utils.parseEther(test.ether.replace(/,/g, '')).eq(wei), 'parsing ether failed - ' + test.name);
        });
        it(('formats ' + wei.toString() + ' wei to ether'), function () {
            var actual = quais_1.quais.utils.formatEther(wei);
        ...
        ['kwei', 'mwei', 'gwei', 'szabo', 'finney', 'satoshi'].forEach(function (name) {
            ...
            if (test[name]) {
                it(('parses ' + test[name] + ' ' + name), function () {
                    this.timeout(120000);
                    assert_1.default.ok(quais_1.quais.utils.parseUnits(test[name].replace(/,/g, ''), unitName).eq(wei), ('parsing ' + name + ' failed - ' + test.name));
                });
            }
            var expectedKey = (name + '_format');
            if (test[expectedKey]) {
                it(('formats ' + wei.toString() + ' wei to ' + name + ')'), function () {
                    var actual = quais_1.quais.utils.formatUnits(wei, unitName);
    ...
    it("formats with commify", function () {
        ...
        Object.keys(tests).forEach(function (test) {
            assert_1.default.equal(quais_1.quais.utils.commify(test), tests[test]);
        });
    });
    // See #2016; @TODO: Add more tests along these lines
    it("checks extra tests", function () {
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2", 0).eq(2), "folds trailing zeros without decimal: 2");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.", 0).eq(2), "folds trailing zeros without decimal: 2.");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.0", 0).eq(2), "folds trailing zeros without decimal: 2.0");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.00", 0).eq(2), "folds trailing zeros without decimal: 2.00");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2", 1).eq(20), "folds trailing zeros: 2");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.", 1).eq(20), "folds trailing zeros: 2.");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.0", 1).eq(20), "folds trailing zeros: 2.0");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.00", 1).eq(20), "folds trailing zeros: 2.00");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.5", 1).eq(25), "folds trailing zeros: 2.5");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.50", 1).eq(25), "folds trailing zeros: 2.50");
        assert_1.default.ok(quais_1.quais.utils.parseUnits("2.500", 1).eq(25), "folds trailing zeros: 2.500");
    });
});
describe('Test Namehash', function () {
    ...
    tests.forEach(function (test) {
        it(('computes namehash - "' + test.name + '"'), function () {
            this.timeout(120000);
            assert_1.default.equal(quais_1.quais.utils.namehash(test.name), test.expected, 'computes namehash(' + test.name + ')');
        });
    });
    var goodNames = [
        "ricmoo.eth",
...
describe('Test ID Hash Functions', function () {
    ...
    tests.forEach(function (test) {
        it(('computes id - ' + test.name), function () {
            this.timeout(120000);
            var actual = quais_1.quais.utils.id(test.text);
...
describe('Test Hash Functions', function () {
    ...
    it('computes keccak256 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(quais_1.quais.utils.keccak256(test.data), test.keccak256, ('Keccak256 - ' + test.data));
        });
    });
    it('computes sha2-256 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(quais_1.quais.utils.sha256(test.data), test.sha256, ('SHA256 - ' + test.data));
        });
    });
    it('computes sha2-512 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(quais_1.quais.utils.sha512(test.data), test.sha512, ('SHA512 - ' + test.data));
        });
    });
});
describe('Test Solidity splitSignature', function () {
    it('splits a canonical signature', function () {
        ...
        for (var v = 27; v <= 28; v++) {
            var signature = quais_1.quais.utils.concat([r, s, [v]]);
            var sig = quais_1.quais.utils.splitSignature(signature);
    ...
    it('splits a legacy signature', function () {
        ...
        var s = '0xcafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7e';
        for (var v = 27; v <= 28; v++) {
            var signature = quais_1.quais.utils.concat([r, s, [v - 27]]);
            var sig = quais_1.quais.utils.splitSignature(signature);
            assert_1.default.equal(sig.r, r, 'split r correctly');
            assert_1.default.equal(sig.s, s, 'split s correctly');
            assert_1.default.equal(sig.v, v, 'split v correctly');
...
describe('Test Base64 coder', function () {
    ...
    it('encodes and decodes the example from wikipedia', function () {
        ...
        var decoded = quais_1.quais.utils.toUtf8Bytes(decodedText);
        ...
        assert_1.default.equal(quais_1.quais.utils.base64.encode(decoded), encoded, 'encodes to base64 string');
        assert_1.default.equal(quais_1.quais.utils.toUtf8String(quais_1.quais.utils.base64.decode(encoded)), decodedText, 'decodes from base64 string');
    });
});
describe('Test UTF-8 coder', function () {
    var overlong = quais_1.quais.utils.Utf8ErrorReason.OVERLONG;
    var utf16Surrogate = quais_1.quais.utils.Utf8ErrorReason.UTF16_SURROGATE;
    var overrun = quais_1.quais.utils.Utf8ErrorReason.OVERRUN;
    var missingContinue = quais_1.quais.utils.Utf8ErrorReason.MISSING_CONTINUE;
    var unexpectedContinue = quais_1.quais.utils.Utf8ErrorReason.UNEXPECTED_CONTINUE;
    var outOfRange = quais_1.quais.utils.Utf8ErrorReason.OUT_OF_RANGE;
    var BadUTF = [
    ...
    BadUTF.forEach(function (test) {
        it('toUtf8String - ' + test.name, function () {
            ...
            var ignored = quais_1.quais.utils.toUtf8String(test.bytes, quais_1.quais.utils.Utf8ErrorFuncs.ignore);
            assert_1.default.equal(ignored, test.ignored, "ignoring errors matches");
            // Check the string using the replaceErrors conversion
            var replaced = quais_1.quais.utils.toUtf8String(test.bytes, quais_1.quais.utils.Utf8ErrorFuncs.replace);
            ...
            assert_1.default.throws(function () {
                var result = quais_1.quais.utils.toUtf8String(test.bytes);
    ...
    it('toUtf8String - random conversions', function () {
        ...
        for (var i = 0; i < 100000; i++) {
            ...
            var bytes = quais_1.quais.utils.toUtf8Bytes(str);
            var str2 = quais_1.quais.utils.toUtf8String(bytes);
            var escaped = JSON.parse(quais_1.quais.utils._toEscapedUtf8String(bytes));
...
describe('Test Bytes32String coder', function () {
    ...
    it("encodes an ens name", function () {
        ...
        var bytes32 = quais_1.quais.utils.formatBytes32String(str);
        var str2 = quais_1.quais.utils.parseBytes32String(bytes32);
...
function getHex(value) {
    return quais_1.quais.utils.hexlify(quais_1.quais.utils.toUtf8Bytes(value));
}
describe("Test nameprep", function () {
    ...
    Tests.forEach(function (test) {
        ...
        it(test.comment, function () {
            var input = quais_1.quais.utils.toUtf8String(test.input);
            if (test.output) {
...
describe("Test Signature Manipulation", function () {
    ...
    tests.forEach(function (test) {
        it("autofills partial signatures - " + test.name, function () {
            var address = quais_1.quais.utils.getAddress(test.accountAddress);
            var hash = quais_1.quais.utils.keccak256(test.unsignedTransaction);
            var data = quais_1.quais.utils.RLP.decode(test.signedTransaction);
            var s = data.pop(), r = data.pop(), v = parseInt(data.pop().substring(2), 16);
            var sig = quais_1.quais.utils.splitSignature({ r: r, s: s, v: v });
            {
                var addr = quais_1.quais.utils.recoverAddress(hash, {
                    r: r, s: s, v: v
                });
...
describe("Test Typed Transactions", function () {
    ...
    function equalsData(name, a, b, ifNull) {
        assert_1.default.equal(quais_1.quais.utils.hexlify(a), quais_1.quais.utils.hexlify((b == null) ? ifNull : b), name);
        return true;
    }
    function equalsNumber(name, a, b, ifNull) {
        assert_1.default.ok(quais_1.quais.BigNumber.from(a).eq((b == null) ? ifNull : b), name);
        return true;
    }
        ...
        it(test.name, function () {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            wallet = new quais_1.quais.Wallet(test.key);
...
describe("BigNumber", function () {
    ...
    tests.forEach(function (test) {
        if (test.expectedValue == null) {
            ...
            it(test.testcase, function () {
                ...
                var value2 = quais_1.quais.BigNumber.from(value);
    ...
    it("Fails on junk with a length property", function () {
        ...
        assert_1.default.throws(function () {
            var value = quais_1.quais.BigNumber.from("100").add(junk);
            console.log("ERROR", value);
        }, function (error) {
            return true;
        });
...
describe("Logger", function () {
    var logger = new quais_1.quais.utils.Logger("testing/0.0");
    it("setLogLevel", function () {
        quais_1.quais.utils.Logger.setLogLevel(quais_1.quais.utils.Logger.levels.DEBUG);
        quais_1.quais.utils.Logger.setLogLevel(quais_1.quais.utils.Logger.levels.INFO);
        quais_1.quais.utils.Logger.setLogLevel(quais_1.quais.utils.Logger.levels.WARNING);
        quais_1.quais.utils.Logger.setLogLevel(quais_1.quais.utils.Logger.levels.ERROR);
        quais_1.quais.utils.Logger.setLogLevel(quais_1.quais.utils.Logger.levels.OFF);
    ...
    it("checkArgumentCount - too few", function () {
        assert_1.default.throws(function () {
            ...
            return error.code === quais_1.quais.utils.Logger.errors.MISSING_ARGUMENT;
        });
    });
    it("checkArgumentCount - too many", function () {
        assert_1.default.throws(function () {
            ...
            return error.code === quais_1.quais.utils.Logger.errors.UNEXPECTED_ARGUMENT;
        });
    });
});
describe("Base58 Coder", function () {
    it("decodes", function () {
        assert_1.default.equal(quais_1.quais.utils.toUtf8String(quais_1.quais.utils.base58.decode("JxF12TrwUP45BMd")), "Hello World");
    });
    it("encodes", function () {
        assert_1.default.equal(quais_1.quais.utils.base58.encode(quais_1.quais.utils.toUtf8Bytes("Hello World")), "JxF12TrwUP45BMd");
    });
});
/*
...
describe("EIP-712", function () {
    ...
    tests.forEach(function (test) {
        it("encoding " + test.name, function () {
            ...
            //console.log(test);
            assert_1.default.equal(quais_1.quais.utils._TypedDataEncoder.getPrimaryType(test.types), test.primaryType, "getPrimaryType");
            assert_1.default.equal(quais_1.quais.utils._TypedDataEncoder.hash(test.domain, test.types, test.data), test.digest, "digest");
        });
    });
    tests.forEach(function (test) {
        if (!test.privateKey) {
        ...
        it("signing " + test.name, function () {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            wallet = new quais_1.quais.Wallet(test.privateKey);
    ...
    var Tests = [
        {
            ...
            tx: {
                ...
                chainId: 3,
                nonce: 13,
                maxFeePerGas: quais_1.quais.BigNumber.from("0x143"),
                gasPrice: quais_1.quais.BigNumber.from("0x143"),
                gasLimit: quais_1.quais.BigNumber.from("0x5b68"),
                to: "0x32162F3581E88a5f62e8A61892B42C46E2c18f7b",
                value: quais_1.quais.BigNumber.from("0"),
                data: "0x",
                accessList: [
                    {
        ...
        },
        {
            ...
            tx: {
                ...
                chainId: 3,
                nonce: 14,
                maxPriorityFeePerGas: quais_1.quais.BigNumber.from("0x42"),
                maxFeePerGas: quais_1.quais.BigNumber.from("0x143"),
                gasPrice: quais_1.quais.BigNumber.from("0x143"),
                gasLimit: quais_1.quais.BigNumber.from("0x71ac"),
                to: "0x32162F3581E88a5f62e8A61892B42C46E2c18f7b",
                value: quais_1.quais.BigNumber.from("0"),
                data: "0x",
                accessList: [
